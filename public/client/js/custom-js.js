$(document).ready(function () {
    ...
    var sqlInjectionPattern = /^[a-zA-ZÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểếệỉịọỏốồổỗộớờởỡợụủứừửữựỳỵỷỹđĐ' -]+$/u;
    ...
    $("#login-form").on("submit", function (e) {
        ...
        // Kiểm tra tên đăng nhập và mật khẩu không chứa ký tự đặc biệt (SQL injection)
         ...
         if (!emailPattern.test(email)) {
            isValid = false;
            $("#validate_email").show().text("Email không hợp lệ.");
        }
    ...
    $("#register-form").on("submit", function (e) {
        ...
        var name = $("#name_register").val().trim();
        var email = $("#email_register").val().trim();
        var password = $("#pass_register").val().trim();
        var rePass = $("#re_pass").val().trim();

        // Đặt lại nội dung thông báo lỗi và ẩn chúng
        $("#validate_name").hide().text("");
        $("#validate_email").hide().text("");
        $("#validate_pass").hide().text("");
        ...
        if (!sqlInjectionPattern.test(name)) {
            ...
            $("#validate_name")
                .show()
                .text("Họ tên không được chứa ký tự đặc biệt.");
        }
                ...
                error: function (xhr, textStatus, errorThrown) {
                    if (xhr.status === 422) {
                        ...
                        if (errors.email_register) {
                            $("#validate_email").show().text(errors.email_register);
                        }
                    } else {
                        toastr.error("Có lỗi xảy ra. Vui lòng thử lại sau.");
                    }
